// @leetup=custom
// @leetup=info id=1829 lang=rust slug=maximum-xor-for-each-query
...
*
* 1. Find a non-negative integer `k < 2maximumBit` such that `nums[0] XOR nums[1]
...
impl Solution {
    pub fn get_maximum_xor(nums: Vec<i32>, maximum_bit: i32) -> Vec<i32> {
        ...
        let max = i32::pow(2, maximum_bit as u32) - 1;
        let mut answer: Vec<i32> = Vec::with_capacity(n);
        for i in (0..nums.len()).rev() {
            answer.push(sum_xor ^ max);
            sum_xor ^= nums[i];
        }
        answer
    }
