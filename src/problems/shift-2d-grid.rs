// @leetup=custom
// @leetup=info id=1260 lang=rust slug=shift-2d-grid
...
impl Solution {
    pub fn shift_grid(grid: Vec<Vec<i32>>, k: i32) -> Vec<Vec<i32>> {
        ...
        let mut flat = grid.iter().flatten().copied().collect::<Vec<_>>();
        flat.rotate_right(k as usize % (m * n));
        flat.chunks(n).map(|x| x.to_vec()).collect()
    }
}
// @leetup=codew
